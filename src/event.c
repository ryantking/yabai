...
static void window_did_receive_focus(struct window_manager *wm,
...
static EVENT_CALLBACK(EVENT_HANDLER_APPLICATION_LAUNCHED) {
  ...
  if (process->terminated) {
    debug("%s: %s (%d) terminated during launch\n", __FUNCTION__, process->name,
          process->pid);
    window_manager_remove_lost_front_switched_event(&g_window_manager,
                                                    process->pid);
    return EVENT_FAILURE;
  }

  if (!workspace_application_is_observable(process)) {
    debug("%s: %s (%d) is not observable, subscribing to activationPolicy "
          "changes\n",
          __FUNCTION__, process->name, process->pid);
    workspace_application_observe_activation_policy(g_workspace_context,
                                                    process);
    return EVENT_FAILURE;
  }

  if (!workspace_application_is_finished_launching(process)) {
    debug("%s: %s (%d) is not finished launching, subscribing to "
          "finishedLaunching changes\n",
          __FUNCTION__, process->name, process->pid);
    workspace_application_observe_finished_launching(g_workspace_context,
                                                     process);
    return EVENT_FAILURE;
  }
    ...
    if (ax_retry) {
      ...
      dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.1f * NSEC_PER_SEC),
                     dispatch_get_main_queue(), ^{
                       struct process *_process = process_manager_find_process(
                           &g_process_manager, &psn);
                       if (_process)
                         event_loop_post(&g_event_loop, APPLICATION_LAUNCHED,
                                         _process, 0, NULL);
                     });
    }
  ...
  if (window_manager_find_lost_front_switched_event(&g_window_manager,
                                                    process->pid)) {
  ...
  window_manager_add_application_windows(&g_space_manager, &g_window_manager,
                                         application);
  event_signal_push(SIGNAL_APPLICATION_LAUNCHED, application);

  int window_count;
  struct window **window_list = window_manager_find_application_windows(
      &g_window_manager, application, &window_count);
    ...
    if (window_manager_should_manage_window(window)) {
      ...
      struct view *view =
          space_manager_tile_window_on_space_with_insertion_point(
              &g_space_manager, window, sid, prev_window_id);
...
static EVENT_CALLBACK(EVENT_HANDLER_APPLICATION_TERMINATED) {
  ...
  struct application *application =
      window_manager_find_application(&g_window_manager, process->pid);
...
static EVENT_CALLBACK(EVENT_HANDLER_APPLICATION_FRONT_SWITCHED) {
  ...
  if (!application) {
    window_manager_add_lost_front_switched_event(&g_window_manager,
                                                 process->pid);
    return EVENT_FAILURE;
  }

  event_loop_post(&g_event_loop, APPLICATION_DEACTIVATED,
                  (void *)(intptr_t)g_process_manager.front_pid, 0, NULL);
  event_loop_post(&g_event_loop, APPLICATION_ACTIVATED,
                  (void *)(intptr_t)process->pid, 0, NULL);
...
static EVENT_CALLBACK(EVENT_HANDLER_APPLICATION_ACTIVATED) {
  struct application *application = window_manager_find_application(
      &g_window_manager, (pid_t)(intptr_t)context);
  if (!application)
    return EVENT_FAILURE;
  ...
  uint32_t application_focused_window_id =
      application_focused_window(application);
  ...
  struct window *window = window_manager_find_window(
      &g_window_manager, application_focused_window_id);
  if (!window) {
    window_manager_add_lost_focused_event(&g_window_manager,
...
static EVENT_CALLBACK(EVENT_HANDLER_APPLICATION_DEACTIVATED) {
  ...
  struct window *focused_window = window_manager_find_window(
      &g_window_manager, application_focused_window(application));
  if (!focused_window)
    return EVENT_SUCCESS;

  window_manager_set_window_opacity(&g_window_manager, focused_window,
                                    g_window_manager.normal_window_opacity);
  border_deactivate(focused_window);

  if (!window_level_is_standard(focused_window) ||
      !window_is_standard(focused_window)) {
    struct window *main_window = window_manager_find_window(
        &g_window_manager, application_main_window(application));
    if (main_window && main_window != focused_window) {
      window_manager_set_window_opacity(&g_window_manager, main_window,
                                        g_window_manager.normal_window_opacity);
...
static EVENT_CALLBACK(EVENT_HANDLER_APPLICATION_VISIBLE) {
  ...
  for (int i = 0; i < window_count; ++i) {
    ...
    if (window_manager_should_manage_window(window)) {
      struct view *view =
          space_manager_tile_window_on_space_with_insertion_point(
              &g_space_manager, window, window_space(window), prev_window_id);
      window_manager_add_managed_window(&g_window_manager, window, view);
      prev_window_id = window->id;
    }
  }
...
static EVENT_CALLBACK(EVENT_HANDLER_WINDOW_CREATED) {
  ...
  struct application *application =
      window_manager_find_application(&g_window_manager, window_pid);
  if (!application)
    goto err;

  struct window *window = window_manager_create_and_add_window(
      &g_space_manager, &g_window_manager, application, context, window_id);
  if (!window)
    goto out;
  ...
  if (window_manager_should_manage_window(window) &&
    ...
    if (!sid) /* if (g_window_manager.window_origin_mode ==
    ...
    struct view *view =
        space_manager_tile_window_on_space(&g_space_manager, window, sid);
    window_manager_add_managed_window(&g_window_manager, window, view);
  }
...
static EVENT_CALLBACK(EVENT_HANDLER_WINDOW_MOVED) {
  ...
  if (window->application->is_hidden) {
    debug(
        "%s: %d was moved while the application is hidden, ignoring event..\n",
        __FUNCTION__, window_id);
    return EVENT_FAILURE;
  }
  ...
  if (CGPointEqualToPoint(new_origin, window->frame.origin)) {
    debug("%s:DEBOUNCED %s %d\n", __FUNCTION__, window->application->name,
          window->id);
    return EVENT_FAILURE;
  }
...
static EVENT_CALLBACK(EVENT_HANDLER_WINDOW_RESIZED) {
  ...
  if (window->application->is_hidden) {
    debug("%s: %d was resized while the application is hidden, ignoring "
          "event..\n",
          __FUNCTION__, window_id);
    return EVENT_FAILURE;
  }
  ...
  if (!was_fullscreen && is_fullscreen) {
    ...
    if (window_manager_should_manage_window(window) &&
        !window_manager_find_managed_window(&g_window_manager, window)) {
      struct view *view = space_manager_tile_window_on_space(
          &g_space_manager, window, window_space(window));
      window_manager_add_managed_window(&g_window_manager, window, view);
    }

    border_exit_fullscreen(window);
    window_manager_make_window_topmost(&g_window_manager, window,
                                       window->is_floating);
  } else if (!was_fullscreen == !is_fullscreen) {
...
static EVENT_CALLBACK(EVENT_HANDLER_WINDOW_DEMINIMIZED) {
  ...
  if (space_manager_is_window_on_space(sid, window)) {
    ...
    if (window_manager_should_manage_window(window) &&
        !window_manager_find_managed_window(&g_window_manager, window)) {
      struct window *last_window = window_manager_find_window(
          &g_window_manager, g_window_manager.last_window_id);
      uint32_t insertion_point = last_window && last_window->application->pid !=
                                                    window->application->pid
                                     ? last_window->id
                                     : 0;
      struct view *view =
          space_manager_tile_window_on_space_with_insertion_point(
              &g_space_manager, window, sid, insertion_point);
      window_manager_add_managed_window(&g_window_manager, window, view);
    }
  } else {
  ...
  if (window_manager_find_lost_focused_event(&g_window_manager, window->id)) {
    event_loop_post(&g_event_loop, WINDOW_FOCUSED, (void *)(intptr_t)window->id,
                    0, NULL);
    window_manager_remove_lost_focused_event(&g_window_manager, window->id);
  }
...
static EVENT_CALLBACK(EVENT_HANDLER_SPACE_CHANGED) {
  ...
  struct view *view = space_manager_find_view(&g_space_manager,
                                              g_space_manager.current_space_id);

  if (space_manager_refresh_application_windows(&g_space_manager)) {
    struct window *focused_window =
        window_manager_focused_window(&g_window_manager);
    if (focused_window && window_manager_find_lost_focused_event(
                              &g_window_manager, focused_window->id)) {
      window_did_receive_focus(&g_window_manager, &g_mouse_state,
                               focused_window);
      window_manager_remove_lost_focused_event(&g_window_manager,
                                               focused_window->id);
    }
  }
  ...
  if (space_is_user(g_space_manager.current_space_id)) {
    ...
    window_manager_validate_and_check_for_windows_on_space(
        &g_space_manager, &g_window_manager, g_space_manager.current_space_id);
  }
...
static EVENT_CALLBACK(EVENT_HANDLER_DISPLAY_CHANGED) {
  ...
  g_space_manager.last_space_id = g_space_manager.current_space_id;
  g_space_manager.current_space_id =
      display_space_id(g_display_manager.current_display_id);

  uint32_t expected_display_id =
      space_display_id(g_space_manager.current_space_id);
  if (g_display_manager.current_display_id != expected_display_id) {
    debug("%s: %d %lld did not match %d! ignoring event..\n", __FUNCTION__,
          g_display_manager.current_display_id,
          g_space_manager.current_space_id, expected_display_id);
    return EVENT_FAILURE;
  }
...
static EVENT_CALLBACK(EVENT_HANDLER_DISPLAY_ADDED) {
  ...
  window_manager_handle_display_add_and_remove(&g_space_manager,
                                               &g_window_manager, did);
...
static EVENT_CALLBACK(EVENT_HANDLER_MOUSE_DOWN) {
  ...
  struct window *window =
      window_manager_find_window_at_point(&g_window_manager, point);
  if (!window)
    window = window_manager_focused_window(&g_window_manager);
  if (!window || window_is_fullscreen(window))
    goto out;
...
static EVENT_CALLBACK(EVENT_HANDLER_MOUSE_UP) {
  ...
  if (!__sync_bool_compare_and_swap(&g_mouse_state.window->id_ptr,
  ...
  if (window_is_fullscreen(g_mouse_state.window)) {
    debug("%s: %d is transitioning into native-fullscreen mode, ignoring "
          "event..\n",
          __FUNCTION__, g_mouse_state.window->id);
    goto err;
  }
  ...
  struct view *src_view = window_manager_find_managed_window(
  ...
  if (info.changed_position && !info.changed_size) {
    uint64_t cursor_sid =
        display_space_id(display_manager_point_display_id(point));
    struct view *dst_view =
        space_manager_find_view(&g_space_manager, cursor_sid);

    struct window *window =
        window_manager_find_window_at_point_filtering_window(
            &g_window_manager, point, g_mouse_state.window->id);
    if (!window)
      window = window_manager_find_window_at_point(&g_window_manager, point);
    if (window == g_mouse_state.window)
      window = NULL;

    struct window_node *a_node =
        view_find_window_node(src_view, g_mouse_state.window->id);
    struct window_node *b_node =
        window ? view_find_window_node(dst_view, window->id) : NULL;

    if (a_node && b_node && a_node != b_node) {
      enum mouse_drop_action drop_action =
          mouse_determine_drop_action(&g_mouse_state, a_node, window, point);
      switch (drop_action) {
      case MOUSE_DROP_ACTION_STACK: {
        mouse_drop_action_stack(&g_space_manager, &g_window_manager, src_view,
                                g_mouse_state.window, dst_view, window);
      } break;
      case MOUSE_DROP_ACTION_SWAP: {
        mouse_drop_action_swap(&g_window_manager, src_view, a_node,
                               g_mouse_state.window, dst_view, b_node, window);
      } break;
      case MOUSE_DROP_ACTION_WARP_TOP: {
        mouse_drop_action_warp(&g_space_manager, &g_window_manager, src_view,
                               a_node, g_mouse_state.window, dst_view, b_node,
                               window, SPLIT_X, CHILD_FIRST);
      } break;
      case MOUSE_DROP_ACTION_WARP_RIGHT: {
        mouse_drop_action_warp(&g_space_manager, &g_window_manager, src_view,
                               a_node, g_mouse_state.window, dst_view, b_node,
                               window, SPLIT_Y, CHILD_SECOND);
      } break;
      case MOUSE_DROP_ACTION_WARP_BOTTOM: {
        mouse_drop_action_warp(&g_space_manager, &g_window_manager, src_view,
                               a_node, g_mouse_state.window, dst_view, b_node,
                               window, SPLIT_X, CHILD_SECOND);
      } break;
      case MOUSE_DROP_ACTION_WARP_LEFT: {
        mouse_drop_action_warp(&g_space_manager, &g_window_manager, src_view,
                               a_node, g_mouse_state.window, dst_view, b_node,
                               window, SPLIT_Y, CHILD_FIRST);
      } break;
      ...
      mouse_drop_no_target(&g_space_manager, &g_window_manager, src_view,
                           dst_view, g_mouse_state.window, a_node);
    }
  } else {
    mouse_drop_try_adjust_bsp_grid(&g_window_manager, src_view,
                                   g_mouse_state.window, &info);
  }
...
static EVENT_CALLBACK(EVENT_HANDLER_MOUSE_DRAGGED) {
  ...
  if (g_mouse_state.current_action == MOUSE_MODE_MOVE) {
    CGPoint new_point = {g_mouse_state.window_frame.origin.x +
                             (point.x - g_mouse_state.down_location.x),
                         g_mouse_state.window_frame.origin.y +
    ...
    scripting_addition_move_window(g_mouse_state.window->id, new_point.x,
                                   new_point.y);
  } else if (g_mouse_state.current_action == MOUSE_MODE_RESIZE) {
    uint64_t event_time = CGEventGetTimestamp(context);
    float dt =
        ((float)event_time - g_mouse_state.last_moved_time) * (1.0f / 1E6);
    if (dt < 66.67f)
      goto out;
    ...
    window_manager_resize_window_relative_internal(g_mouse_state.window, frame,
                                                   direction, dx, dy);
...
static EVENT_CALLBACK(EVENT_HANDLER_MOUSE_MOVED) {
  ...
  if (g_mission_control_active)
    goto out;
  if (g_mouse_state.ffm_window_id)
    goto out;
  ...
  if (window) {
    ...
    if (g_window_manager.ffm_mode == FFM_AUTOFOCUS) {
      ...
      CFArrayRef window_list =
          SLSCopyAssociatedWindows(g_connection, window->id);
      int window_count = CFArrayGetCount(window_list);

      uint32_t child_wid;
      for (int i = 0; i < window_count; ++i) {
        CFNumberGetValue(CFArrayGetValueAtIndex(window_list, i),
                         kCFNumberSInt32Type, &child_wid);
        struct window *child =
            window_manager_find_window(&g_window_manager, child_wid);
        if (!child)
          continue;
        ...
        bool valid =
            CFEqual(role, kAXSheetRole) || CFEqual(role, kAXDrawerRole);
        CFRelease(role);
      ...
      window_manager_focus_window_without_raise(&window->application->psn,
                                                window->id);
      ...
      // the window below the cursor we do not actually perform the focus
      // change, as it is likely that the user is trying to reach for the
      ...
      int window_count;
      uint32_t *window_list = space_window_list(
          g_space_manager.current_space_id, &window_count, false);
      ...
      for (int i = 0; i < window_count; ++i) {
        ...
        struct window *sub_window =
            window_manager_find_window(&g_window_manager, wid);
        if (!sub_window)
          continue;
    ...
    if (cgrect_contains_point(menu, point))
      goto out;
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.1f * NSEC_PER_SEC),
                 dispatch_get_main_queue(), ^{
                   event_loop_post(&g_event_loop,
                                   MISSION_CONTROL_CHECK_FOR_EXIT, NULL, 0,
                                   NULL);
                 });
...
static EVENT_CALLBACK(EVENT_HANDLER_MISSION_CONTROL_CHECK_FOR_EXIT) {
  ...
  if (found) {
    ...
    dispatch_after(
        dispatch_time(DISPATCH_TIME_NOW, 0.0f), dispatch_get_main_queue(), ^{
...
static EVENT_CALLBACK(EVENT_HANDLER_MISSION_CONTROL_EXIT) {
  ...
  if (g_mission_control_active == 1 || g_mission_control_active == 2) {
    window_manager_correct_for_mission_control_changes(&g_space_manager,
                                                       &g_window_manager);
  }
...
static EVENT_CALLBACK(EVENT_HANDLER_DOCK_DID_RESTART) {
  ...
  if (!workspace_is_macos_monterey() && !workspace_is_macos_bigsur() &&
    ...
    for (int window_index = 0; window_index < g_window_manager.window.capacity;
         ++window_index) {
...
static EVENT_CALLBACK(EVENT_HANDLER_DAEMON_MESSAGE) {
  ...
  while ((bytes_read = read(param1, buffer, sizeof(buffer) - 1)) > 0) {
    ...
    if (((message + cursor)[-1] == '\0') && ((message + cursor)[-2] == '\0')) {
