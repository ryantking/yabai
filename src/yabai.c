...
#define CONNECTION_CALLBACK(name)                                              \
  void name(uint32_t type, void *data, size_t data_length, void *context,      \
            int cid)
typedef CONNECTION_CALLBACK(connection_callback);
extern CGError SLSRegisterConnectionNotifyProc(int cid,
...
static void parse_arguments(int argc, char **argv) {
  if ((string_equals(argv[1], VERSION_OPT_LONG)) ||
    ...
    fprintf(stdout, "yabai-ryantking-v%d.%d.%d\n", MAJOR, MINOR, PATCH);
    exit(EXIT_SUCCESS);
  }
